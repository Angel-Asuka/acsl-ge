...
export class Camera implements IView{
    ...
    constructor(core:Core, decl: CameraDeclaration = {}){
        this._core = core
    ...
    set position(v: Vector3) { this._position = v; this.update() }

    /**
     * 读写上向量属性
     */
    get up(): Vector3 { return this._upvector }
    set up(v: Vector3) { this._upvector = v; this.update() }

    /**
     * 读写目标属性
     */
    get target(): Vector3 { return this._target }
    set target(v: Vector3) { this._target = v; this.update() }
    ...
    update(target?:IRenderingTarget){
        if(target) this._projMatrix.perspective(this._fov, target.width/target.height, this._near, this._far)
    ...
    activate(target:IRenderingTarget){
        this.update(target)
        this._core.device._current_pv_matrix = this._pvMatrix.data
    }
}